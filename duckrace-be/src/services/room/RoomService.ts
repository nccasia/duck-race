...
class RoomService implements IRoomService {
    ...
    private randomDuckArray(ducks: Duck[]): Duck[] {
        // Implementation here
        return [...ducks].sort(() => Math.random() - 0.5);
    }
    ...
    public async joinRoom(data: IJoinRoomSubmitDTO): Promise<ServiceResponse> {
        try {
            ...
            return await this.getRoomById(data.roomId);
        } catch (error) {
            logger.error(error?.message);
            return {
                statusCode: 500,
        ...
        try {
            ...
            if (currentRoom) {
                ...
                if (currentRoom.members.length === 0) {
                    await this.removeRoom(currentRoom.roomId);
                }
            }
            return {
                statusCode: 200,
    ...
    public async startTurn(roomId: string): Promise<ServiceResponse> {
        try {
            ...
            // Vòng lặp theo thời gian (giây)
            for (let timeStep = 0; timeStep < expiredTime; timeStep++) {
