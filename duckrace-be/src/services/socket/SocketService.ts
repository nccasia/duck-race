...
class SocketService implements ISocketService {
    ...
    onStartGame = async (socket: Socket, data: IStartGameSubmitDTO) => {
        ...
        if (startGameResponse.isSuccess) {
            ...
            const allTurn = await this._roomService.startTurn(data.roomId);
            this.socketServer.to(startGameResponse.data.roomId).emit(SocketEvents.EMIT.START_TURN_SUCCESS, allTurn);
            if (allTurn.isSuccess) {
                ...
                if(check){
                    ...
                    if (endGameResponse.isSuccess) {
                        ...
                        this.socketServer.to(startGameResponse.data.roomId).emit(SocketEvents.EMIT.END_GAME_SUCCESS, dataEmit);
                    }
                }
            }
